...
- [x] 新增 RPG MV/MZ 版本选择（v0）：全局与单游戏各一套，`EngineSettingsStore` / `PerGameSettingsStore` 分键存储，`EngineLauncher.effectiveRpgMakerVersion` 合并优先级 `单游戏覆盖 ?: 全局`，经 `rpgMakerVersion` / `rpgMvVersion` / `rpgMzVersion` extra 透传至 `TyranoActivity`（v0 阶段仅日志与分支占位，不改注入行为）。
- [x] 三仓库对比：`rpgtkoolmv/corescript`（v1.3b，MIT，Pixi 4.5.4，移动端修复完整）选为 v0 基线，vendor 至 `third_party/rpg-maker-mv-corescript-v0/`（见 `VENDOR.md`）；`Archeia` 为同源备选，`siakoMobi` 無 License 已排除。
